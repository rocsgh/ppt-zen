---
name: Portolan Sea Chart
slug: portolan
medium: Portolan chart
hand: ""
world: A voyage across an uncharted sea toward a marked destination
tags: [warm, parchment, watercolor, epic, exploration]
author: ppt-zen
license: CC-BY-4.0
axes:
  material: >
    Aged sepia parchment, muted watercolor sea, ink linework, compass roses,
    faint rhumb lines, a red destination dot, brush-stroke growth curves.
  skeleton: auto
  device_examples: >
    A rising curve drawn as a fleet of sailing ships; an unnamed dotted island
    for a hidden closing symbol; a compass rose for orientation.
prompt_formula: |
  SURFACE:  16th-century portolan sea chart on aged sepia parchment, muted watercolor
            ocean, fine ink coastlines, compass roses, faint rhumb lines. NO photographic, NO 3D.
  SKELETON: auto by page role (cover=flowline, chapter=color-field, list=grid)
  DEVICE:   draw the page's argument as a chart element (a growth curve = a fleet of ships
            climbing a rising sea-lane; scale = a distance measured on the chart)
  END:      Full-bleed 16:9, render DEVICE on the parchment SURFACE, DEVICE composition.
  CRITICAL: title = the page's TEXT in its own language, clean legible serif; NEVER invent words or glyphs beyond TEXT;
            annotations only as compass marks / numbers / dotted rhumb lines; no UI, no cards, no captions.
source: >
  Inspired by Renaissance portolan navigation charts and ink-wash seascapes. Our own artwork.
samples:
  - samples/01.jpg
---

# Portolan Sea Chart

**A voyage across an uncharted sea.** Warm, epic, exploratory — turns a "growth / going somewhere new" story into a fleet sailing a rising sea-lane toward a marked destination.

![sample](samples/01.jpg)
